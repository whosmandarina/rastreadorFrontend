...
export const locationService = {
  sync: async (locations: OfflineLocation[]): Promise<{ puntos_guardados: number }> => {
  ...
  syncPending: async (): Promise<number> => {
    ...
    const locations: OfflineLocation[] = pending.map(({ id, created_at, ...loc }: any) => loc);
    await locationService.sync(locations);
    await offlineDB.clearAll();
    return pending.length;
  },

  saveOrSend: async (loc: OfflineLocation, isOnline: boolean): Promise<void> => {
    if (isOnline) {
      try {
