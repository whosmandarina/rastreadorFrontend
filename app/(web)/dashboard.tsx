...
import {
  geofencesService,
  Geofence,
} from '../../src/services/geofences.service';
...
function buildLeafletHTML(users: LiveUser[], geofences: Geofence[]): string {
  ...
  const geofenceLayers = geofences
    .map((g) => {
      if (g.tipo === 'CIRCLE') {
        ...
        const ll = g.coordenadas
          .map((c: any) => `[${c.lat ?? c.latitude}, ${c.lng ?? c.longitude}]`)
          .join(',');
  ...
  const center = users[0]?.latitud
    ? `[${users[0].latitud}, ${users[0].longitud}]`
    : '[19.2433, -103.7247]';
...
export default function DashboardScreen() {
  ...
  const liveList = Object.values(liveUsers).map((lu) => {
    ...
    return {
      ...lu,
      nombre: dbUser?.nombre ?? lu.nombre ?? `Usuario ${lu.id_user}`,
    };
  });
  ...
  const mapKey =
    liveList
      .map((u) => `${u.id_user}:${u.latitud}:${u.longitud}:${u.status}`)
      .join('|') +
    '|' +
  ...
  const saveGeo = async () => {
    if (!geoForm.nombre.trim()) {
      alert('El nombre es obligatorio');
      return;
    }
    if (geoForm.tipo === 'CIRCLE') {
      if (!geoForm.lat || !geoForm.lng) {
      ...
      if (valid.length < 3) {
        alert('Un polígono necesita mínimo 3 puntos con coordenadas');
        return;
      }
    }
    setSavingGeo(true);
    try {
      ...
      const payload: any = {
        ...
        coordenadas,
        radio:
          geoForm.tipo === 'CIRCLE' ? parseFloat(geoForm.radio) : undefined,
      };
      if (editingGeo)
        await geofencesService.update(editingGeo.id_geofence, payload);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSub}>
            {liveList.filter((u) => u.status === 'ONLINE').length} activos ·{' '}
            {recentAlerts.length} alertas · {geofences.length} geocercas
          </Text>
        </View>
        <View style={styles.headerRight}>
          {isAdmin && (
            <TouchableOpacity
              style={[
                styles.geoToggleBtn,
                showGeoPanel && styles.geoToggleBtnActive,
              ]}
              onPress={() => setShowGeoPanel(!showGeoPanel)}
            >
              <Text
                style={[
                  styles.geoToggleText,
                  showGeoPanel && styles.geoToggleTextActive,
                ]}
              >
                📍 Geocercas
              </Text>
          ...
          >
            ...
            <Text
              style={[
                styles.connText,
                { color: connected ? COLORS.success : COLORS.accent },
              ]}
            >
              {connected ? 'En línea' : 'Sin socket'}
            </Text>
          </View>
        </View>
      </View>
        ...
        <View style={styles.mapContainer}>
          ...
          {recentAlerts.length > 0 && (
            <View style={styles.alertsOverlay}>
              <Text style={styles.alertsOverlayTitle}>
                🔔 Alertas recientes
              </Text>
              {recentAlerts.slice(0, 3).map((a, i) => (
                <Text key={i} style={styles.alertOverlayText} numberOfLines={1}>
                  • {a.tipo_alerta} — {a.descripcion}
                </Text>
              ))}
            </View>
          )}
                ...
                <View key={g.id_geofence} style={styles.legendItem}>
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          g.tipo === 'CIRCLE' ? COLORS.primary : COLORS.warning,
                      },
                    ]}
                  />
          ...
          {showGeoPanel ? (
            <>
              <View style={styles.panelHeader}>
                ...
                <TouchableOpacity style={styles.addBtn} onPress={openCreateGeo}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  <View key={g.id_geofence} style={styles.geoCard}>
                    <View style={styles.geoCardLeft}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.geoCardMeta}>
                          {g.tipo === 'CIRCLE' ? 'Círculo' : 'Polígono'}
                          {g.radio ? ` · ${g.radio}m` : ''}
                        </Text>
                      </View>
                    </View>
                    <View style={styles.geoCardActions}>
                      <TouchableOpacity
                        style={styles.geoActionBtn}
                        onPress={() => openEditGeo(g)}
                      >
                        <Text>✏️</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        ...
                        onPress={() => setConfirmDeleteGeo(g.id_geofence)}
                      >
                        <Text>🗑</Text>
                      </TouchableOpacity>
                    </View>
                ...
                {geofences.length === 0 && (
                  <View style={styles.emptyGeo}>
                    ...
                    />
                    <Text style={styles.emptyGeoText}>
                      Sin geocercas. Crea una para comenzar.
                    </Text>
                  </View>
                )}
              </ScrollView>
            ...
            <>
                  ...
                  <View style={styles.dashDropdownMenu}>
                    {[
                      ...
                      <TouchableOpacity
                        key={r.value}
                        style={[
                          styles.dashDropdownItem,
                          filterRol === r.value &&
                            styles.dashDropdownItemActive,
                        ]}
                        onPress={() => {
                          setFilterRol(r.value);
                  ...
                  {enriched.map((u) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.userCard,
                          selectedUser === u.id_user && styles.userCardActive,
                        ]}
                        onPress={() =>
                          setSelectedUser(
                            u.id_user === selectedUser ? null : u.id_user,
                          )
                        }
                      >
              ...
              {selectedUser !== null && liveUsers[selectedUser] && (
                <View style={styles.userDetail}>
                  ...
                  {[
                    ['Batería', `${liveUsers[selectedUser].bateria ?? '—'}%`],
                    [
                      'Velocidad',
                      `${liveUsers[selectedUser].velocidad ?? 0} km/h`,
                    ],
                    ['Señal', liveUsers[selectedUser].senal ?? '—'],
                    [
                      'Lat/Lng',
                      `${liveUsers[selectedUser].latitud?.toFixed(4)}, ${liveUsers[selectedUser].longitud?.toFixed(4)}`,
                    ],
                  ].map(([l, v]) => (
      ...
      <ConfirmModal
        ...
        onConfirm={() =>
          confirmDeleteGeo !== null && deleteGeo(confirmDeleteGeo)
        }
        ...
        <View style={styles.overlay}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>
              {editingGeo ? 'Editar' : 'Nueva'} Geocerca
            </Text>
            <ScrollView
            ...
            >
              ...
              <View style={styles.mfield}>
                ...
                <TextInput
                  style={styles.minput}
                  value={geoForm.nombre}
                  onChangeText={(v) => setGeoForm((p) => ({ ...p, nombre: v }))}
                ...
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  {(['CIRCLE', 'POLYGON'] as const).map((t) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.typeBtn,
                        geoForm.tipo === t && styles.typeBtnActive,
                      ]}
                      onPress={() => {
                        setGeoForm((p) => ({ ...p, tipo: t }));
                      }}
                    >
                      <Text
                        style={[
                          styles.typeBtnText,
                          geoForm.tipo === t && { color: COLORS.primary },
                        ]}
                      >
              ...
              {geoForm.tipo === 'POLYGON' && (
                <View style={styles.mfield}>
                  ...
                  >
                    <Text style={styles.mfieldLabel}>
                      Puntos del polígono * (mínimo 3)
                    </Text>
                    <TouchableOpacity
                      style={{
                  ...
                  >
                    ...
                    >
                      💡 Ingresa las coordenadas de cada vértice del polígono en
                      orden. Puedes obtenerlas desde Google Maps haciendo clic
                      derecho → "¿Qué hay aquí?".
                    </Text>
                  </View>
                </View>
              )}
