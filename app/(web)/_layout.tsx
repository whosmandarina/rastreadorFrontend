...
const NAV_ITEMS = [
  { label: 'Dashboard',    icon: 'map-outline',          iconActive: 'map',              route: '/(web)/dashboard',  roles: ['ADMIN', 'SUPERVISOR'] },
  { label: 'Usuarios',     icon: 'people-outline',       iconActive: 'people',           route: '/(web)/users',       roles: ['ADMIN', 'SUPERVISOR'] },
...
export default function WebLayout() {
  ...
  const allowedNav = NAV_ITEMS.filter((item) => item.roles.includes(user?.rol || ''));
    ...
    <View style={styles.shell}>
      <View style={styles.sidebar}>
        ...
        <View style={styles.statusRow}>
          <View style={[styles.dot, { backgroundColor: connected ? COLORS.primaryLight : COLORS.danger }]} />
          <Text style={styles.statusText}>{connected ? 'En línea' : 'Desconectado'}</Text>
        </View>

        <ScrollView style={styles.nav} showsVerticalScrollIndicator={false}>
          {allowedNav.map((item) => {
            const routeKey = item.route.replace('/(web)/', '');
            const isActive = pathname === `/${routeKey}` || pathname.endsWith(routeKey);
            return (
              <TouchableOpacity
                key={item.route}
              ...
              >
                <Ionicons name={(isActive ? item.iconActive : item.icon) as any} size={18} color={isActive ? COLORS.textOnDark : '#6b9ab8'} />
...
const styles = StyleSheet.create({
  ...
  sidebar: { width: 220, backgroundColor: COLORS.bgDark, paddingTop: 24, paddingHorizontal: 12 },
  brand: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 8, marginBottom: 20 },
  logoCircle: { width: 36, height: 36, borderRadius: 10, backgroundColor: COLORS.primary, alignItems: 'center', justifyContent: 'center' },
  logoIcon: { fontSize: 18, color: '#fff' },
  logoText: { fontSize: 16, fontWeight: '800', color: COLORS.textOnDark, letterSpacing: 1.5, textTransform: 'uppercase' },
  statusRow: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 8, marginBottom: 20 },
  ...
  userSection: { borderTopWidth: 1, borderTopColor: '#0a2d42', paddingTop: 12, paddingBottom: 24, gap: 10 },
  ...
  logoutBtn: { backgroundColor: COLORS.danger + '20', borderWidth: 1, borderColor: COLORS.danger + '50', borderRadius: 8, paddingVertical: 8, alignItems: 'center' },
