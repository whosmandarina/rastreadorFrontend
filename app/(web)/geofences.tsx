...
export default function GeofencesScreen() {
  ...
  const save = async () => {
    ...
    const nameError = geoValidators.nombre(form.nombre);
    if (nameError) errs.nombre = nameError;

    if (form.tipo === 'CIRCLE') {
      const latError = geoValidators.latitud(form.lat);
      if (latError) errs.lat = latError;
      const lngError = geoValidators.longitud(form.lng);
      if (lngError) errs.lng = lngError;
      const radioError = geoValidators.radio(form.radio);
      if (radioError) errs.radio = radioError;
    } else {
      const validPoints = polyPoints.filter(p => p.lat.trim() && p.lng.trim());
      if (validPoints.length < 3) errs.puntos = 'Un polígono necesita mínimo 3 puntos con coordenadas';
      else {
        for (let i = 0; i < validPoints.length; i++) {
          const ptError = geoValidators.puntoPoligono(validPoints[i].lat, validPoints[i].lng, i);
          if (ptError) { errs.puntos = ptError; break; }
        }
      }
    }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={openCreate}>
          <Text style={styles.addBtnText}><Ionicons name="add-outline" size={14} color="#fff" /> Nueva geocerca</Text>
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.grid}>
            {geofences.map((g) => (
              <View key={g.id_geofence} style={styles.card}>
                <Ionicons name={g.tipo === 'CIRCLE' ? 'radio-button-on-outline' : 'shapes-outline'} size={28} color={g.tipo === 'CIRCLE' ? COLORS.primary : COLORS.warning} />
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.field}>
                ...
                <TextInput
                  ...
                  value={form.nombre}
                  onChangeText={(v) => { setForm((p) => ({ ...p, nombre: v })); setFieldErrors(p => ({...p, nombre: undefined})); }}
                  placeholder="Zona Planta Norte"
                  placeholderTextColor={COLORS.textMuted}
                />
                {fieldErrors.nombre && <Text style={styles.fieldError}>⚠️ {fieldErrors.nombre}</Text>}
              </View>
                  ...
                  {(['CIRCLE', 'POLYGON'] as const).map((t) => (
                    ...
                    >
                      <Text style={[styles.typeBtnText, form.tipo === t && { color: COLORS.primary }]}>
                        <Ionicons name={t === 'CIRCLE' ? 'radio-button-on-outline' : 'shapes-outline'} size={14} /> {t === 'CIRCLE' ? 'Círculo' : 'Polígono'}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
                ...
                <>
                  <View style={styles.row2}>
                    <View style={[styles.field, { flex: 1 }]}>
                      ...
                      <TextInput style={[styles.input, fieldErrors.lat && styles.inputError]} value={form.lat} onChangeText={(v) => { setForm((p) => ({ ...p, lat: v })); setFieldErrors(p => ({...p, lat: undefined})); }}
                        placeholder="19.2433" placeholderTextColor={COLORS.textMuted} keyboardType="numeric" />
                      {fieldErrors.lat && <Text style={styles.fieldError}>⚠️ {fieldErrors.lat}</Text>}
                    </View>
                    <View style={[styles.field, { flex: 1 }]}>
                      <Text style={styles.fieldLabel}>Longitud *</Text>
                      <TextInput style={[styles.input, fieldErrors.lng && styles.inputError]} value={form.lng} onChangeText={(v) => { setForm((p) => ({ ...p, lng: v })); setFieldErrors(p => ({...p, lng: undefined})); }}
                        placeholder="-103.7247" placeholderTextColor={COLORS.textMuted} keyboardType="numeric" />
                      {fieldErrors.lng && <Text style={styles.fieldError}>⚠️ {fieldErrors.lng}</Text>}
                    </View>
                  </View>
                  <View style={styles.field}>
                    <Text style={styles.fieldLabel}>Radio (metros) *</Text>
                    <TextInput style={[styles.input, fieldErrors.radio && styles.inputError]} value={form.radio} onChangeText={(v) => { setForm((p) => ({ ...p, radio: v })); setFieldErrors(p => ({...p, radio: undefined})); }}
                      placeholder="150" placeholderTextColor={COLORS.textMuted} keyboardType="numeric" />
                    {fieldErrors.radio && <Text style={styles.fieldError}>⚠️ {fieldErrors.radio}</Text>}
                  </View>
                </>
              )}
              ...
              {form.tipo === 'POLYGON' && (
                <View style={styles.field}>
                  <View style={styles.polyHeader}>
                    ...
                    >
                      <Text style={styles.addPointText}><Ionicons name="add-outline" size={12} color="#fff" /> Punto</Text>
                    </TouchableOpacity>
                  </View>
                  ...
                  {fieldErrors.puntos && <Text style={styles.fieldError}>⚠️ {fieldErrors.puntos}</Text>}
