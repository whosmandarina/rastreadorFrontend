...
const isValidRadius = (value: number) =>
  ...
  value >= VALIDATION_LIMITS.geofenceRadiusMin &&
  value <= VALIDATION_LIMITS.geofenceRadiusMax;
...
export default function GeofencesScreen() {
  ...
  const [polyPoints, setPolyPoints] =
    useState<{ lat: string; lng: string }[]>(EMPTY_POINTS);
  ...
  const save = async () => {
    ...
    if (form.nombre.trim().length > VALIDATION_LIMITS.geofenceNameMax) {
      setError(
        `geofence name must have at most ${VALIDATION_LIMITS.geofenceNameMax} characters`,
      );
      return;
    }
    ...
    if (form.tipo === 'CIRCLE') {
      ...
      if (!isValidRadius(radius)) {
        setError(
          `radius must be between ${VALIDATION_LIMITS.geofenceRadiusMin} and ${VALIDATION_LIMITS.geofenceRadiusMax} meters`,
        );
        return;
      }
      ...
      if (valid.length < 3) {
        setError('polygon must have at least 3 valid points');
        return;
      }
      ...
      const hasInvalidPoint = parsedPoints.some(
        (point) => !isValidLatitude(point.lat) || !isValidLongitude(point.lng),
      );
      if (hasInvalidPoint) {
        setError(
          'all polygon points must be within valid latitude and longitude ranges',
        );
        return;
      }
          ...
          <View style={styles.grid}>
            {geofences.map((g) => (
              <View key={g.id_geofence} style={styles.card}>
                ...
                {g.radio && (
                  <Text style={styles.cardMeta}>Radio: {g.radio}m</Text>
                )}
                ...
                <View style={styles.cardActions}>
                  ...
                  <TouchableOpacity
                    style={styles.delBtn}
                    onPress={() => setConfirmDelete(g.id_geofence)}
                  >
            ...
            {geofences.length === 0 && (
              <View style={styles.empty}>
                ...
                <Text style={styles.emptyText}>
                  No hay geocercas. Crea una para comenzar.
                </Text>
              </View>
            )}
          </View>
                ...
                <View style={styles.typeRow}>
                  {(['CIRCLE', 'POLYGON'] as const).map((t) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.typeBtn,
                        form.tipo === t && styles.typeBtnActive,
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          styles.typeBtnText,
                          form.tipo === t && { color: COLORS.primary },
                        ]}
                      >
                  ...
                  <View style={styles.field}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(v) =>
                        setForm((p) => ({
                          ...p,
                          radio: sanitizeCoordinateInput(v),
                        }))
                      }
                      placeholder="150"
                      placeholderTextColor={COLORS.textMuted}
              ...
              {form.tipo === 'POLYGON' && (
                <View style={styles.field}>
                  ...
                  <View style={styles.polyColHeader}>
                    ...
                    <Text style={[styles.polyColText, { flex: 1 }]}>
                      Latitud
                    </Text>
                    <Text style={[styles.polyColText, { flex: 1 }]}>
                      Longitud
                      ...
                      <TextInput
                        ...
                        onChangeText={(v) =>
                          setPolyPoints((prev) =>
                            prev.map((p, idx) =>
                              idx === i
                                ? { ...p, lat: sanitizeCoordinateInput(v) }
                                : p,
                            ),
                          )
                        }
                      ...
                      <TextInput
                        ...
                        onChangeText={(v) =>
                          setPolyPoints((prev) =>
                            prev.map((p, idx) =>
                              idx === i
                                ? { ...p, lng: sanitizeCoordinateInput(v) }
                                : p,
                            ),
                          )
                        }
                  ...
                  <View style={styles.tipBox}>
                    <Text style={styles.tipText}>
                      💡 Obtén coordenadas en Google Maps: clic derecho → "¿Qué
                      hay aquí?"
                    </Text>
                  </View>
                </View>
