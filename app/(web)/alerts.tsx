...
export default function AlertsScreen() {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      usersData.forEach((u: User) => { map[u.id_user] = u.nombre; });
      setUsers(map);
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          {unread.length > 0 && (
            <>
              ...
              {unread.map((a) => <AlertCard key={a.id_alert} alert={a} onRead={markRead} users={users} />)}
            </>
          )}
          {read.length > 0 && (
            <>
              <Text style={styles.section}>Leídas</Text>
              {read.map((a) => <AlertCard key={a.id_alert} alert={a} onRead={markRead} users={users} />)}
            </>
          )}
...
function AlertCard({ alert, onRead, users }: { alert: Alert; onRead: (id: number) => void; users: Record<number, string> }) {
  ...
  const typeLabel = ALERT_TYPE_LABELS[alert.tipo_alerta] ?? alert.tipo_alerta.replace(/_/g, ' ');
  const userName = (alert as any).usuario_nombre || alert.nombre || (alert.id_user ? users[alert.id_user] : null);
...
const styles = StyleSheet.create({
  ...
  cardUserRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 3, marginBottom: 3 },
